...
describe("protocol registry — posture", () => {
  ...
  it("only active+built protocols contribute Move targets", () => {
    ...
    // Built adapters: Cetus + DeepBook + aggregator (swap) + NAVI/Suilend (lending) + Wormhole (bridge).
    expect(built).toEqual(["cetus", "cetus-aggregator", "deepbook", "navi", "suilend", "wormhole"]);
...
describe("protocol registry — single-authored allowlist", () => {
  it("ALLOWED_MOVE_TARGETS = CORE_TARGETS ∪ active-built targets (exact, no drift)", () => {
    ...
    // Backstop on the exact size: 4 core (pay::split_and_transfer, transfer::
    // public_share_object, coin::destroy_zero, registry::lookup) + 4 Cetus + 6 DeepBook
    ...
    // + 1 Wormhole (complete_transfer) = 28.
    expect(ALLOWED_MOVE_TARGETS.size).toBe(28);
  });
