...
14. **Composite recipe gate** (`checkCompositeRecipe`, atomic single-sign only) — enforces four invariants for declared multi-action recipes (dynamic recipe supporting send/swap/lend/stake combinations with optional chaining): (a) closed-recipe registry (only allowlisted actions in the dynamic recipe; ad-hoc composition refused), (b) target multiset (exact PTB MoveCall set; send legs emit zero calls — verified by absence + matching balance deltas), (c) coin-type linkage (for chained legs, output coin type of leg k-1 must equal input coin type of leg k), (d) recipient-aware anti-leak via `checkCompositeDeltaAntiLeak` (every third-party inflow must match a declared send leg's recipient/coinType/amount multiset; coin objectChanges safely skipped; no-send-leg path zero-tolerance) + dual caps (USD cap + net-SUI cap). Any failure → BLOCK.
...
**Multi-step chaining** — a compound intent ("swap 5 SUI to USDC then lend it on NAVI" or "send 1 to @alice and send 1 to @bob") is detected via Vietnamese connectors (rồi / và sau đó / tiếp theo) + `isChainableSequence` parser; if valid, routes to a chain-plan card instead of refusal. Each step is a normal single-action PTB; the `PlanStepper` state machine (packages/agent/src/chaining/plan-stepper.ts) handles delta resolution (step k+1 consumes output of step k, not pre-existing balance), stale-object waits (avoid "unavailable for consumption"), and halt semantics (BLOCK at step k cancels later steps). Page refresh loses in-flight chain state (durable resume not yet implemented); a transient stale-object error auto-rebuilds the step with fresh bytes (bounded, never re-sends stale bytes). **Atomic single-sign composite is LIVE (generalized dynamic recipe):** the builder (`buildDynamicComposite`) composes ANY ordered sequence of allowlisted actions (send, swap, lend_deposit, stake haSUI) into ONE PTB with optional chaining — the output of leg k-1 can feed the input of leg k structurally. No wallet round-trip between legs. An upfront SUI-coverage gate (`assertSuiGasCoverage`) catches shortfalls as `insufficient_gas`. The Guardian's `checkCompositeRecipe` re-verifies the whole PTB via four gates: (a) closed-recipe registry (only allowlisted actions), (b) target multiset (exact MoveCall set per leg), (c) coin-type linkage (output/input coin types match for chained legs), (d) recipient-aware anti-leak + dual caps. On any build/Guardian failure it degrades to the sequential chain — funds and every check unaffected. See `atomic-composite-mode.md`.
