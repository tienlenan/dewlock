/**
 * tech-logos.tsx — inline brand marks for the AI side of the "Powered by" strip.
 *
 * Mastra and Gemini use their official marks (LobeHub / simple-icons),
 * monochrome via `currentColor` so they sit in the same ink as the Sui-stack
 * lockups and flip cleanly in dark mode. The Sui / Walrus / Memwal lockups are
 * served as static SVGs from /public/logos instead.
 *
 * Marks render on a 24×24 viewBox and inherit size + colour from the parent.
 */
...
/** Google Gemini — official sparkle (simple-icons), monochrome. */
...
/** Mastra — official mark (LobeHub icons), monochrome via currentColor. */
export function MastraLogo({ className, style }: LogoProps) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" fillRule="evenodd" aria-hidden className={className} style={style}>
...
/** Inline marks keyed by COPY.poweredBy item.key (img lockups handle the rest). */
export const TECH_MARKS = {
  mastra: MastraLogo,
  gemini: GeminiLogo,
} as const;

export type TechMarkKey = keyof typeof TECH_MARKS;
